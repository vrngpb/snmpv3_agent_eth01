...
while true; do
    ...
    clear
    echo "=== WT32-ETH01 SNMP Monitor  $(date '+%H:%M:%S') ==="
    echo ""
    echo "  Температура : ${TEMP_C} °C"
    echo "  Протечка    : ${LEAK_S}"
    echo "  Дверь 1     : ${D1_S}"
    echo "  Дверь 2     : ${D2_S}"
    echo ""
    echo "  Heap free   : ${HEAP_FREE} байт  (занято ${HEAP_PCT}%)"
    echo "  Uptime      : ${UPTIME} ticks"
    echo ""
    echo "  IP          : ${IP}"
    echo "  Gateway     : ${GW}"
    echo "  Netmask     : ${MASK}"
